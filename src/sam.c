...
// Insert at WX or YX following, copying the stress
        ...
        if (debug) if (A==20) fprintf(stderr,"RULE: insert WX following diphtong NOT ending in IY sound\n");
        if (debug) if (A==21) fprintf(stderr,"RULE: insert YX following diphtong ending in IY sound\n");
...
// VOWELS AND DIPHTONGS ENDING WITH IY SOUND flag set?
                A = flags[Y] & 32;
                if (debug) if (A==0) fprintf(stderr,"RULE: K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR DIPHTONG NOT ENDING WITH IY>\n");
// Replace with KX
...
// Is next phoneme a vowel or ER?
                A = flags[A] & 128;
            if (debug) if (A != 0) fprintf(stderr,"RULE: Soften T or D following vowel or ER and preceding a pause -> DX\n");
