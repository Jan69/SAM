...
// Insert at WX or YX following, copying the stress
        ...
        if (debug) if (A1==20) fprintf(stderr,"RULE: insert WX following diphtong NOT ending in IY sound\n");
        if (debug) if (A1==21) fprintf(stderr,"RULE: insert YX following diphtong ending in IY sound\n");
...
// VOWELS AND DIPHTONGS ENDING WITH IY SOUND flag set?
                A1 = flags[Y1] & 32;
                if (debug) if (A1==0) fprintf(stderr,"RULE: K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR DIPHTONG NOT ENDING WITH IY>\n");
// Replace with KX
...
// Is next phoneme a vowel or ER?
                A1 = flags[A1] & 128;
            if (debug) if (A1 != 0) fprintf(stderr,"RULE: Soften T or D following vowel or ER and preceding a pause -> DX\n");
...
pos48644:
        ...
        do
        {
            ...
            if(((flags2[index] & 32) == 0) || ((flags[index] & 4) != 0))     //nochmal �berpr�fen
            {
                ...
                A1 = (A1 >> 1) + A1 + 1;
if (debug) fprintf(stderr,"RULE: Lengthen <FRICATIVE> or <VOICED> between <VOWEL> and <PUNCTUATION> by 1.5\n");
if (debug) fprintf(stderr,"PRE\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]], phonemeLength[X1]);

                phonemeLength[X1] = A1;

if (debug) fprintf(stderr,"POST\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]], phonemeLength[X1]);
...
if (debug) fprintf(stderr,"RULE: <VOWEL> <RX | LX> <CONSONANT> - decrease length by 1\n");
if (debug) fprintf(stderr,"PRE\n");
...
if (debug) fprintf(stderr,"RULE: <VOWEL> <UNVOICED PLOSIVE> - decrease vowel by 1/8th\n");
if (debug) fprintf(stderr,"PRE\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]],  phonemeLength[X1]);
...
if (debug) fprintf(stderr,"RULE: <VOWEL> <VOICED CONSONANT> - increase vowel by 1/2 + 1\n");
if (debug) fprintf(stderr,"PRE\n");
...
//pos48821:
        ...
        if((flags2[index] & 8) != 0)
        {
            ...
            {
if (debug) fprintf(stderr,"RULE: <NASAL> <STOP CONSONANT> - set nasal = 5, consonant = 6\n");
if (debug) fprintf(stderr,"POST\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]], phonemeLength[X1]);
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1-1, signInputTable1[phonemeindex[X1-1]], signInputTable2[phonemeindex[X1-1]], phonemeLength[X1-1]);
...
if (debug) fprintf(stderr,"RULE: <UNVOICED STOP CONSONANT> {optional silence} <STOP CONSONANT> - shorten both to 1/2 + 1\n");
if (debug) fprintf(stderr,"PRE\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]], phonemeLength[X1]);
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1-1, signInputTable1[phonemeindex[X1-1]], signInputTable2[phonemeindex[X1-1]], phonemeLength[X1-1]);
...
if (debug) fprintf(stderr,"RULE: <LIQUID CONSONANT> <DIPHTONG> - decrease by 2\n");
if (debug) fprintf(stderr,"PRE\n");
if (debug) fprintf(stderr,"phoneme %d (%c%c) length %d\n", X1, signInputTable1[phonemeindex[X1]], signInputTable2[phonemeindex[X1]], phonemeLength[X1]);
